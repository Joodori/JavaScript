...
// /person/list가 요청경로일 때 이 경로로 요청이 들어오면 설정한 콜백함수를 실행함
router.route('/person/list').get(async (req, res) => {
    ...
    try {
        ...
        // 콜백함수의 두번째 구멍으로 전달해줌
        req.app.render('list', context, (err, html) => { // views라는 폴더 안에 있는 list.ejs라는 확장자를 읽어줄게 (나는 저 파일을 html로 썼어. 그래서 콜백함수에 html을 넣어줄게)
...
// person/insert 요청경로일 때 이 경로로 요청이 들어오면 설정한 콜백함수를 실행함
router.route('/person/insert').get(async (req, res) => {
    console.log(`/person/insert 요청경로로 요청됨`)
    ...
    try {
        ...
        // 업데이트한 결과로 고객 목록을 볼 수 있도록 list.ejs 웹페이지 파일을 불러온 후 DB의 데이터와 결합해서 응답 보내기
...
// /person/add 요청경로일 때 이 경로로 요청이 들어오면 설정한 콜백함수를 실행함
router.route('/person/add').get(async (req, res) => {
    console.log(`/person/add 요청경로로 요청됨`)
    ...
    try {
        ...
        // views 폴더 안에 있는 update.ejs 웹페이지 파일을 읽어오고 요청 파라미터로 전달받은 데이터를 결합한 후 
        // 콜백함수의 두번째 구멍으로 전달해줌
        req.app.render('add', context, (err, html) => { // views라는 폴더 안에 있는 list.ejs라는 확장자를 읽어줄게 (나는 저 파일을 html로 썼어. 그래서 콜백함수에 html을 넣어줄게)
            // 만약 첫번째 구멍으로 에러가 전달된다면
            if (err) {
                console.log(`뷰 처리 중 에러 -> ${err}`);
                return;
...
// /person/modify 요청경로일 때 이 경로로 요청이 들어오면 설정한 콜백함수를 실행함
...
// /person/update 요청경로일 때 이 경로로 요청이 들어오면 설정한 콜백함수를 실행함
router.route('/person/update').get(async (req, res) => {
    ...
    try {
        ...
        // views 폴더 안에 있는 update.ejs 웹페이지 파일을 읽어오고 요청 파라미터로 전달받은 데이터를 결합한 후 
        // 콜백함수의 두번째 구멍으로 전달해줌
        req.app.render('update', context, (err, html) => { // views라는 폴더 안에 있는 list.ejs라는 확장자를 읽어줄게 (나는 저 파일을 html로 썼어. 그래서 콜백함수에 html을 넣어줄게)
            // 만약 첫번째 구멍으로 에러가 전달된다면
            if (err) {
                console.log(`뷰 처리 중 에러 -> ${err}`);
                return;
...
// /person/delete 요청경로일 때 이 경로로 요청이 들어오면 설정한 콜백함수를 실행함
router.route('/person/delete').get(async (req, res) => {
    console.log(`/person/delete 요청경로로 요청됨`)

    // 클라이언트로부터 전달받은 요청 파라미터 확인하기
    const params = req.query;
    console.log(`trash 요청 파라미터 -> params : ${JSON.stringify(params)}`);
